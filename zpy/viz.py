...
def plotter(func):
    ...
    def wrapped(
        output_path: Union[Path, str] = None, show: bool = False, **kwargs,
...
@plotter
def image_grid_plot(
    images: List[np.ndarray] = None, rows: int = 4, cols: int = 4,
...
def draw_annotations(
    ...
    DPI = fig.get_dpi()
    fig.set_size_inches(
        image.shape[1] / float(DPI), image.shape[0] / float(DPI),  # width  # height
    )
...
def draw_segmentation(
    ax: matplotlib.axes.Axes, segmentation: List, color: Tuple[int], alpha: float = 0.6,
) -> None:
