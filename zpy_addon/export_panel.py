...
class CleanUpDirOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        zpy.files.remove_files_with_suffix(
            path=context.scene.zpy_export_path, exts=[".blend1", ".blend2", ".blend3"],
        )
...
class ExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        zpy.files.write_json(
            export_path / "_meta.zumo.json", zpy.blender.scene_information(),
        )
...
class SCENE_PT_ExportPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            "scene.zpy_export_sim", text="Export Sim", icon="EXPORT",
        )
        row = layout.row()
        ...
        row = layout.row()
        row.operator(
            "scene.zpy_open_export_dir", text="Open", icon="FILEBROWSER",
        )
        row.operator(
            "scene.zpy_cleanup_sim", text="Clean", icon="PACKAGE",
        )
